import React from "react";
import Cap from "../../assets/icons/Cap.svg";
function TeacherDetailsCard({ teacherDetails }) {
  return (
    <div className="max-w-sm rounded overflow-hidden shadow-lg rounded-lg">
      <div className="px-6 py-4">
        <img
          className="w-25 rounded-full h-25 mx-auto mb-4"
          src="data:image/jpeg;base64,"
        />
        <div className="font-bold text-center text-md mb-2">
          The Coldest Sunset
        </div>
        <div className="text-center text-sm mb-2">email@gmail.com</div>
        <div className="text-center text-sm mb-2">+920000000000</div>
        <div className="flex justify-center mb-2">
          <div className="px-3 py-1 text-sm font-normal rounded-full text-center text-emerald-500 bg-emerald-100/60 ">
            Active
          </div>
        </div>
        <div className="border-t mt-4 border-gray-300"></div>
        <div className="flex justify-between mt-4">
          <div className="font-bold text-left text-sm mb-2">Country</div>
          <div className="text-right text-sm mb-2">Country</div>
        </div>
        <div className="flex justify-between mt-4">
          <div className="font-bold text-left text-sm mb-2">Date Joined</div>
          <div className="text-right text-sm mb-2">Country</div>
        </div>
        <div className="flex justify-between mt-4">
          <div className="font-bold text-left text-sm mb-2">
            Total Amount Paid
          </div>
          <div className="text-right text-sm mb-2">Country</div>
        </div>
        <div className="flex justify-between mt-4">
          <div className="font-bold text-left text-sm mb-2">Payment Status</div>
          <div className="px-3 py-2 text-sm font-bold rounded-full text-center text-emerald-500 bg-emerald-100/60 ">
            Paid
          </div>
        </div>
        <div className="flex justify-between mt-4">
          <div className="font-bold text-left text-sm mb-2">Semester</div>
          <div className="text-right text-sm mb-2">Country</div>
        </div>
        <div className="flex flex-col mt-4">
          <div className="font-semibold  text-left text-md mb-2">Courses</div>
          <div className="flex flex-row justify-between text-teal-600 gap-2">
            <div className="flex-1 text-center text-sm mb-2 font-semibold border-2 border-teal-600 rounded-full p-2">
              Maths
            </div>
            <div className="flex-1 text-center text-sm font-semibold mb-2 border-2 border-teal-600 rounded-full p-2">
              Computer
            </div>
            <div className="flex-1 text-center text-sm mb-2 font-semibold border-2 border-teal-600 rounded-full p-2">
              Science
            </div>
          </div>
        </div>
        <div className="border-t mt-4 border-gray-300"></div>
        <div className="mt-4">
          <div className="flex justify-between">
            <text>
              <span className="font-bold text-left text-lg mt-2">12</span>
            </text>
            <div className="px-3 py-4 text-sm font-bold rounded-full text-center text-emerald-500 bg-[#fff] opacity-30">
              <img src={Cap} alt="Cap" />
            </div>
          </div>
          <div className="text-left text-sm">Materials Subscribed</div>
        </div>
      </div>
    </div>
  );
}

export default TeacherDetailsCard;
